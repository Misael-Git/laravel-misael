                ...
                <div class="p-6">
                    ...
                    <form action="{{ route('tasks.update', $task) }}" method="POST">
                        ...
                        <div class="mb-6 flex items-center gap-2">
                            <input type="checkbox" id="auto_complete" name="auto_complete" value="1" {{ old('auto_complete', $task->auto_complete) ? 'checked' : '' }}
                    ...
                    <script>
                        function initMap() {
                            const initialLat = {{ old('lat', $task->lat ?? auth()->user()->lat ?? 40.4168) }};
                            const initialLng = {{ old('lng', $task->lng ?? auth()->user()->lng ?? -3.7038) }};
                            ...
                            if ({{ old('lat', $task->lat) ? 'true' : 'false' }}) {
                                marker = new google.maps.Marker({
                                    position: {
                                        lat: {{ old('lat', $task->lat ?? 'null') }},
                                        lng: {{ old('lng', $task->lng ?? 'null') }} 
                                    },
                                    map: map
                                });
                            }
